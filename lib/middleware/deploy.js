...
module.exports = function (req, next) {
  ...
  if (req.argv.a) headers['add'] = req.argv.a;

  /**
   * Collaborators to remove
   */

  if (req.argv.r) headers['rem'] = req.argv.r;

  /**
   * Perform build on server
   */

  if (req.argv.build) headers['build'] = req.argv.build;

  /**
   * Force Protocol?
   */

  if (req.ssl !== null) headers['ssl'] = req.ssl;
  ...
  var tick = function (tick) {
    ...
    if (payload.hasOwnProperty('type') && payload.type === 'error') {
    ...
    } else if (payload.hasOwnProperty('type') && payload.type === 'users') {
      helpers.log(helpers.smart('users:').grey, payload.users.join(', '));
    } else if (payload.hasOwnProperty('type') && payload.type === 'collect') {
      //console.log("payload:collect", payload)
      var msg =
        ('   ' + payload.plan.name.underline + ' plan requred. ').grey +
        ('$' + payload.plan.amount / 100 + '/mo with a ' + payload.plan.trial_period_days + ' day trial').underline.grey;
      ...
      // prompt for user
    } else if (payload.hasOwnProperty('type') && payload.type === 'cert') {
      try {
    ...
    } else if (payload.hasOwnProperty('type') && payload.type === 'ip') {
      req.success = true;
      if (payload.data) {
        helpers.log(helpers.smart('IP:').grey + ' xxx.xxx.xxx.xxx');
      }
    } else if (payload.hasOwnProperty('type') && payload.type === 'event_registration') {
      if (payload.data) {
        // console.log();
        console.log(('             ' + payload.data.event.name + ' is set!').bold);

        if (payload.data.event.start) console.log('              start:'.grey, payload.data.event.start);

        if (payload.data.event.end) console.log('                end:'.grey, payload.data.event.end);

        if (payload.data.event.website) console.log('            website:'.grey, payload.data.event.website);

        if (payload.data.event.email) console.log('              email:'.grey, payload.data.event.email);

        if (payload.data.event.twitter) console.log('            twitter:'.grey, payload.data.event.twitter);
      }
    } else if (payload.hasOwnProperty('type') && payload.type === 'event_participant') {
      if (payload.data) {
        // console.log();

        // end time
        var out = ['      You are in '.grey + payload.data.event.name.bold + '!'.grey];
        if (payload.data.event.endTime) out.push('Competition ends in '.grey + payload.data.event.endTime.toString().green);
        ...
        if (payload.data.event.twitter) out.push('  t: '.grey, payload.data.event.twitter);
      ...
      try {
        ...
        progress[payload.id] =
          progress[payload.id] ||
          new ProgressBar(displays[payload.id], {
            complete: '',
            incomplete: '',
            width: 0,
            total: payload.total,
  ...
  handshake.on('response', function (rsp) {
    if (rsp.statusCode == 403) {
      ...
      if (rsp.headers.hasOwnProperty('reason')) {
        ...
        helpers.trunc('Aborted'.yellow + (' - Domain already taken ' + req.domain.underline).grey);
      }
      helpers.log();
      process.exit(1);
