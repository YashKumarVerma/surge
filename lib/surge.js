...
module.exports = function(config) {
  ...
  var ep = config.endpoint ? config.endpoint : config.platform ? 'https://surge.' + config.platform : 'https://surge.surge.sh';

  config.platform = config.platform || 'surge.sh';
  config.name = config.name || 'Mercury';
  ...
  //   this function returns the currently logged in item
  var authInfo = function(req, next) {
    var str = 'By order of '.grey + 'Hephaestus '.red + 'YashKumarVerma'.grey;
    // helpers.space();
    helpers.trunc(str);
    // helpers.space();
    console.log(str);
  ...
  surge.token = function(hooks) {
    ...
    var onion = [whitelist, endpoint, pkg, help, version, space, preAuth, creds, auth, postAuth, token, space];
    return function() {
  ...
  surge.login = function(hooks) {
    ...
    var preAuth = hooks.preAuth || stub;
    var postAuth = hooks.postAuth || stub;
    var onion = [whitelist, endpoint, pkg, help, version, space, preAuth, auth, postAuth, login, space];
    return function() {
      var argv = parse(arguments[arguments.length - 1]);
      skin(
        {
  ...
  surge.logout = function(hooks) {
    var hooks = hooks || {};
    var onion = [whitelist, endpoint, pkg, help, version, space, creds, logout, space];
    return function() {
      var argv = parse(arguments[arguments.length - 1]);
      skin(
        {
  ...
  surge.publish = function(hooks) {
    ...
    var onion = [
      ...
      discovery.suggestDomainFromGenerator,
      domain,
      postDomain,
      prePublish,
      protocol,
  ...
  surge.whoami = function(hooks) {
    var hooks = hooks || {};
    var onion = [whitelist, endpoint, pkg, help, version, space, creds, whoami];
    return function() {
      var argv = parse(arguments[arguments.length - 1]);
      skin(
        {
  ...
  surge.list = function(hooks) {
    ...
    var preAuth = hooks.preAuth || stub;
    var postAuth = hooks.postAuth || stub;
    var onion = [whitelist, endpoint, pkg, help, version, space, preAuth, creds, welcome, auth, postAuth, shorthand, list, space];
    return function() {
      var argv = parse(arguments[arguments.length - 1]);
      skin(
        {
  ...
  surge.teardown = function(hooks) {
    ...
    var onion = [
      ...
      postAuth,
      shorthand,
      discovery.setDomainFromArgs,
      discovery.suggestDomainFromCname,
      domainOrSilent,
      teardown,
      space,
    ];
  ...
  surge.card = function(hooks) {
    ...
    var preAuth = hooks.preAuth || stub;
    var postAuth = hooks.postAuth || stub;
    var onion = [whitelist, endpoint, pkg, help, version, space, preAuth, creds, welcome, auth, postAuth, plans, card, setcard, space];
    return function() {
      var argv = parse(arguments[arguments.length - 1]);
      skin(
        {
